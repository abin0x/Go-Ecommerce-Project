...
func Serve() {
	...
	mux.Handle("GET /abin", middleware.Logger(http.HandlerFunc(handlers.Test)))

	mux.Handle("GET /products", middleware.Logger(http.HandlerFunc(handlers.GetProductsHandler)))
	mux.Handle("POST /products", middleware.Logger(http.HandlerFunc(handlers.CreateProductHandler)))
